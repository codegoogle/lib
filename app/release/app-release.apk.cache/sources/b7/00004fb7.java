package com.p7700g.p99005;

import com.microsoft.appcenter.ingestion.models.properties.DoubleTypedProperty;
import com.microsoft.appcenter.ingestion.models.properties.LongTypedProperty;
import com.p7700g.p99005.aa;
import java.lang.annotation.Annotation;
import java.lang.reflect.Constructor;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Collection;
import java.util.HashMap;
import java.util.LinkedHashMap;
import java.util.List;
import java.util.Map;
import kotlin.jvm.internal.DefaultConstructorMarker;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

/* compiled from: ClassReference.kt */
@vo4(d1 = {"\u0000p\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010 \n\u0002\u0010\u001b\n\u0002\b\u0003\n\u0002\u0010\u001e\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0010\u000b\n\u0002\b\u0016\n\u0002\u0018\u0002\n\u0002\b\u0007\n\u0002\u0010\u000e\n\u0002\b\b\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0002\b\u0006\n\u0002\u0010\u0001\n\u0000\n\u0002\u0010\b\n\u0002\b\u0005\u0018\u0000 O2\b\u0012\u0004\u0012\u00020\u00020\u00012\u00020\u0003:\u0001OB\u0011\u0012\n\u0010\u0004\u001a\u0006\u0012\u0002\b\u00030\u0005¢\u0006\u0002\u0010\u0006J\u0013\u0010F\u001a\u00020\u00122\b\u0010G\u001a\u0004\u0018\u00010\u0002H\u0096\u0002J\b\u0010H\u001a\u00020IH\u0002J\b\u0010J\u001a\u00020KH\u0016J\u0012\u0010L\u001a\u00020\u00122\b\u0010M\u001a\u0004\u0018\u00010\u0002H\u0017J\b\u0010N\u001a\u000201H\u0016R\u001a\u0010\u0007\u001a\b\u0012\u0004\u0012\u00020\t0\b8VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b\n\u0010\u000bR \u0010\f\u001a\u000e\u0012\n\u0012\b\u0012\u0004\u0012\u00020\u00020\u000e0\r8VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b\u000f\u0010\u0010R\u001a\u0010\u0011\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u0013\u0010\u0014\u001a\u0004\b\u0011\u0010\u0015R\u001a\u0010\u0016\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u0017\u0010\u0014\u001a\u0004\b\u0016\u0010\u0015R\u001a\u0010\u0018\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u0019\u0010\u0014\u001a\u0004\b\u0018\u0010\u0015R\u001a\u0010\u001a\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u001b\u0010\u0014\u001a\u0004\b\u001a\u0010\u0015R\u001a\u0010\u001c\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u001d\u0010\u0014\u001a\u0004\b\u001c\u0010\u0015R\u001a\u0010\u001e\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b\u001f\u0010\u0014\u001a\u0004\b\u001e\u0010\u0015R\u001a\u0010 \u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b!\u0010\u0014\u001a\u0004\b \u0010\u0015R\u001a\u0010\"\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b#\u0010\u0014\u001a\u0004\b\"\u0010\u0015R\u001a\u0010$\u001a\u00020\u00128VX\u0097\u0004¢\u0006\f\u0012\u0004\b%\u0010\u0014\u001a\u0004\b$\u0010\u0015R\u0018\u0010\u0004\u001a\u0006\u0012\u0002\b\u00030\u0005X\u0096\u0004¢\u0006\b\n\u0000\u001a\u0004\b&\u0010'R\u001e\u0010(\u001a\f\u0012\b\u0012\u0006\u0012\u0002\b\u00030)0\r8VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b*\u0010\u0010R\u001e\u0010+\u001a\f\u0012\b\u0012\u0006\u0012\u0002\b\u00030\u00010\r8VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b,\u0010\u0010R\u0016\u0010-\u001a\u0004\u0018\u00010\u00028VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b.\u0010/R\u0016\u00100\u001a\u0004\u0018\u0001018VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b2\u00103R(\u00104\u001a\u0010\u0012\f\u0012\n\u0012\u0006\b\u0001\u0012\u00020\u00020\u00010\b8VX\u0097\u0004¢\u0006\f\u0012\u0004\b5\u0010\u0014\u001a\u0004\b6\u0010\u000bR\u0016\u00107\u001a\u0004\u0018\u0001018VX\u0096\u0004¢\u0006\u0006\u001a\u0004\b8\u00103R \u00109\u001a\b\u0012\u0004\u0012\u00020:0\b8VX\u0097\u0004¢\u0006\f\u0012\u0004\b;\u0010\u0014\u001a\u0004\b<\u0010\u000bR \u0010=\u001a\b\u0012\u0004\u0012\u00020>0\b8VX\u0097\u0004¢\u0006\f\u0012\u0004\b?\u0010\u0014\u001a\u0004\b@\u0010\u000bR\u001c\u0010A\u001a\u0004\u0018\u00010B8VX\u0097\u0004¢\u0006\f\u0012\u0004\bC\u0010\u0014\u001a\u0004\bD\u0010E¨\u0006P"}, d2 = {"Lkotlin/jvm/internal/ClassReference;", "Lkotlin/reflect/KClass;", "", "Lkotlin/jvm/internal/ClassBasedDeclarationContainer;", "jClass", "Ljava/lang/Class;", "(Ljava/lang/Class;)V", "annotations", "", "", "getAnnotations", "()Ljava/util/List;", "constructors", "", "Lkotlin/reflect/KFunction;", "getConstructors", "()Ljava/util/Collection;", "isAbstract", "", "isAbstract$annotations", "()V", "()Z", "isCompanion", "isCompanion$annotations", "isData", "isData$annotations", "isFinal", "isFinal$annotations", "isFun", "isFun$annotations", "isInner", "isInner$annotations", "isOpen", "isOpen$annotations", "isSealed", "isSealed$annotations", "isValue", "isValue$annotations", "getJClass", "()Ljava/lang/Class;", "members", "Lkotlin/reflect/KCallable;", "getMembers", "nestedClasses", "getNestedClasses", "objectInstance", "getObjectInstance", "()Ljava/lang/Object;", "qualifiedName", "", "getQualifiedName", "()Ljava/lang/String;", "sealedSubclasses", "getSealedSubclasses$annotations", "getSealedSubclasses", "simpleName", "getSimpleName", "supertypes", "Lkotlin/reflect/KType;", "getSupertypes$annotations", "getSupertypes", "typeParameters", "Lkotlin/reflect/KTypeParameter;", "getTypeParameters$annotations", "getTypeParameters", "visibility", "Lkotlin/reflect/KVisibility;", "getVisibility$annotations", "getVisibility", "()Lkotlin/reflect/KVisibility;", "equals", "other", "error", "", "hashCode", "", "isInstance", "value", "toString", "Companion", "kotlin-stdlib"}, k = 1, mv = {1, 7, 1}, xi = 48)
/* loaded from: classes3.dex */
public final class m15 implements u55<Object>, l15 {
    @NotNull
    public static final a s = new a(null);
    @NotNull
    private static final Map<Class<? extends io4<?>>, Integer> t;
    @NotNull
    private static final HashMap<String, String> u;
    @NotNull
    private static final HashMap<String, String> v;
    @NotNull
    private static final HashMap<String, String> w;
    @NotNull
    private static final Map<String, String> x;
    @NotNull
    private final Class<?> y;

    /* compiled from: ClassReference.kt */
    @vo4(d1 = {"\u00006\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0002\n\u0002\u0010$\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\b\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u000e\n\u0002\u0018\u0002\n\u0002\b\u0007\n\u0002\u0010\u000b\n\u0002\b\u0002\b\u0086\u0003\u0018\u00002\u00020\u0001B\u0007\b\u0002¢\u0006\u0002\u0010\u0002J\u0014\u0010\u000f\u001a\u0004\u0018\u00010\n2\n\u0010\u0010\u001a\u0006\u0012\u0002\b\u00030\u0005J\u0014\u0010\u0011\u001a\u0004\u0018\u00010\n2\n\u0010\u0010\u001a\u0006\u0012\u0002\b\u00030\u0005J\u001c\u0010\u0012\u001a\u00020\u00132\b\u0010\u0014\u001a\u0004\u0018\u00010\u00012\n\u0010\u0010\u001a\u0006\u0012\u0002\b\u00030\u0005R&\u0010\u0003\u001a\u001a\u0012\u0010\u0012\u000e\u0012\n\b\u0001\u0012\u0006\u0012\u0002\b\u00030\u00060\u0005\u0012\u0004\u0012\u00020\u00070\u0004X\u0082\u0004¢\u0006\u0002\n\u0000R*\u0010\b\u001a\u001e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n0\tj\u000e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n`\u000bX\u0082\u0004¢\u0006\u0002\n\u0000R*\u0010\f\u001a\u001e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n0\tj\u000e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n`\u000bX\u0082\u0004¢\u0006\u0002\n\u0000R*\u0010\r\u001a\u001e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n0\tj\u000e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n`\u000bX\u0082\u0004¢\u0006\u0002\n\u0000R\u001a\u0010\u000e\u001a\u000e\u0012\u0004\u0012\u00020\n\u0012\u0004\u0012\u00020\n0\u0004X\u0082\u0004¢\u0006\u0002\n\u0000¨\u0006\u0015"}, d2 = {"Lkotlin/jvm/internal/ClassReference$Companion;", "", "()V", "FUNCTION_CLASSES", "", "Ljava/lang/Class;", "Lkotlin/Function;", "", "classFqNames", "Ljava/util/HashMap;", "", "Lkotlin/collections/HashMap;", "primitiveFqNames", "primitiveWrapperFqNames", "simpleNames", "getClassQualifiedName", "jClass", "getClassSimpleName", "isInstance", "", "value", "kotlin-stdlib"}, k = 1, mv = {1, 7, 1}, xi = 48)
    /* loaded from: classes3.dex */
    public static final class a {
        private a() {
        }

        public /* synthetic */ a(DefaultConstructorMarker defaultConstructorMarker) {
            this();
        }

        @Nullable
        public final String a(@NotNull Class<?> cls) {
            String str;
            c25.p(cls, "jClass");
            String str2 = null;
            if (cls.isAnonymousClass() || cls.isLocalClass()) {
                return null;
            }
            if (!cls.isArray()) {
                String str3 = (String) m15.w.get(cls.getName());
                return str3 == null ? cls.getCanonicalName() : str3;
            }
            Class<?> componentType = cls.getComponentType();
            if (componentType.isPrimitive() && (str = (String) m15.w.get(componentType.getName())) != null) {
                str2 = wo1.t(str, "Array");
            }
            return str2 == null ? "kotlin.Array" : str2;
        }

        @Nullable
        public final String b(@NotNull Class<?> cls) {
            String str;
            c25.p(cls, "jClass");
            String str2 = null;
            if (!cls.isAnonymousClass()) {
                if (cls.isLocalClass()) {
                    String simpleName = cls.getSimpleName();
                    Method enclosingMethod = cls.getEnclosingMethod();
                    if (enclosingMethod != null) {
                        c25.o(simpleName, "name");
                        String p5 = c95.p5(simpleName, enclosingMethod.getName() + h95.c, null, 2, null);
                        if (p5 != null) {
                            return p5;
                        }
                    }
                    Constructor<?> enclosingConstructor = cls.getEnclosingConstructor();
                    if (enclosingConstructor == null) {
                        c25.o(simpleName, "name");
                        return c95.o5(simpleName, h95.c, null, 2, null);
                    }
                    c25.o(simpleName, "name");
                    return c95.p5(simpleName, enclosingConstructor.getName() + h95.c, null, 2, null);
                } else if (!cls.isArray()) {
                    String str3 = (String) m15.x.get(cls.getName());
                    return str3 == null ? cls.getSimpleName() : str3;
                } else {
                    Class<?> componentType = cls.getComponentType();
                    if (componentType.isPrimitive() && (str = (String) m15.x.get(componentType.getName())) != null) {
                        str2 = wo1.t(str, "Array");
                    }
                    if (str2 == null) {
                        return "Array";
                    }
                }
            }
            return str2;
        }

        public final boolean c(@Nullable Object obj, @NotNull Class<?> cls) {
            c25.p(cls, "jClass");
            Map map = m15.t;
            c25.n(map, "null cannot be cast to non-null type kotlin.collections.Map<K of kotlin.collections.MapsKt__MapsKt.get, V of kotlin.collections.MapsKt__MapsKt.get>");
            Integer num = (Integer) map.get(cls);
            if (num != null) {
                return k35.B(obj, num.intValue());
            }
            if (cls.isPrimitive()) {
                cls = yy4.g(yy4.i(cls));
            }
            return cls.isInstance(obj);
        }
    }

    /* JADX WARN: Multi-variable type inference failed */
    static {
        int i = 0;
        List M = js4.M(uz4.class, f05.class, j05.class, k05.class, l05.class, m05.class, n05.class, o05.class, p05.class, q05.class, vz4.class, wz4.class, xz4.class, yz4.class, zz4.class, a05.class, b05.class, c05.class, d05.class, e05.class, g05.class, h05.class, i05.class);
        ArrayList arrayList = new ArrayList(ks4.Z(M, 10));
        for (Object obj : M) {
            int i2 = i + 1;
            if (i < 0) {
                js4.X();
            }
            arrayList.add(cq4.a((Class) obj, Integer.valueOf(i)));
            i = i2;
        }
        t = nt4.B0(arrayList);
        HashMap<String, String> hashMap = new HashMap<>();
        hashMap.put("boolean", "kotlin.Boolean");
        hashMap.put("char", "kotlin.Char");
        hashMap.put("byte", "kotlin.Byte");
        hashMap.put("short", "kotlin.Short");
        hashMap.put("int", "kotlin.Int");
        hashMap.put(aa.b.c, "kotlin.Float");
        hashMap.put(LongTypedProperty.TYPE, "kotlin.Long");
        hashMap.put(DoubleTypedProperty.TYPE, "kotlin.Double");
        u = hashMap;
        HashMap<String, String> hashMap2 = new HashMap<>();
        hashMap2.put("java.lang.Boolean", "kotlin.Boolean");
        hashMap2.put("java.lang.Character", "kotlin.Char");
        hashMap2.put("java.lang.Byte", "kotlin.Byte");
        hashMap2.put("java.lang.Short", "kotlin.Short");
        hashMap2.put("java.lang.Integer", "kotlin.Int");
        hashMap2.put("java.lang.Float", "kotlin.Float");
        hashMap2.put("java.lang.Long", "kotlin.Long");
        hashMap2.put("java.lang.Double", "kotlin.Double");
        v = hashMap2;
        HashMap<String, String> hashMap3 = new HashMap<>();
        hashMap3.put("java.lang.Object", "kotlin.Any");
        hashMap3.put("java.lang.String", "kotlin.String");
        hashMap3.put("java.lang.CharSequence", "kotlin.CharSequence");
        hashMap3.put("java.lang.Throwable", "kotlin.Throwable");
        hashMap3.put("java.lang.Cloneable", "kotlin.Cloneable");
        hashMap3.put("java.lang.Number", "kotlin.Number");
        hashMap3.put("java.lang.Comparable", "kotlin.Comparable");
        hashMap3.put("java.lang.Enum", "kotlin.Enum");
        hashMap3.put("java.lang.annotation.Annotation", "kotlin.Annotation");
        hashMap3.put("java.lang.Iterable", "kotlin.collections.Iterable");
        hashMap3.put("java.util.Iterator", "kotlin.collections.Iterator");
        hashMap3.put("java.util.Collection", "kotlin.collections.Collection");
        hashMap3.put("java.util.List", "kotlin.collections.List");
        hashMap3.put("java.util.Set", "kotlin.collections.Set");
        hashMap3.put("java.util.ListIterator", "kotlin.collections.ListIterator");
        hashMap3.put("java.util.Map", "kotlin.collections.Map");
        hashMap3.put("java.util.Map$Entry", "kotlin.collections.Map.Entry");
        hashMap3.put("kotlin.jvm.internal.StringCompanionObject", "kotlin.String.Companion");
        hashMap3.put("kotlin.jvm.internal.EnumCompanionObject", "kotlin.Enum.Companion");
        hashMap3.putAll(hashMap);
        hashMap3.putAll(hashMap2);
        Collection<String> values = hashMap.values();
        c25.o(values, "primitiveFqNames.values");
        for (String str : values) {
            StringBuilder sb = new StringBuilder();
            sb.append("kotlin.jvm.internal.");
            c25.o(str, "kotlinName");
            sb.append(c95.s5(str, '.', null, 2, null));
            sb.append("CompanionObject");
            hp4 a2 = cq4.a(sb.toString(), str + ".Companion");
            hashMap3.put(a2.m(), a2.n());
        }
        for (Map.Entry<Class<? extends io4<?>>, Integer> entry : t.entrySet()) {
            int intValue = entry.getValue().intValue();
            hashMap3.put(entry.getKey().getName(), "kotlin.Function" + intValue);
        }
        w = hashMap3;
        LinkedHashMap linkedHashMap = new LinkedHashMap(mt4.j(hashMap3.size()));
        for (Map.Entry entry2 : hashMap3.entrySet()) {
            linkedHashMap.put(entry2.getKey(), c95.s5((String) entry2.getValue(), '.', null, 2, null));
        }
        x = linkedHashMap;
    }

    public m15(@NotNull Class<?> cls) {
        c25.p(cls, "jClass");
        this.y = cls;
    }

    @tp4(version = "1.4")
    public static /* synthetic */ void C() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void F() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void I() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void K() {
    }

    @tp4(version = "1.5")
    public static /* synthetic */ void L() {
    }

    private final Void k() {
        throw new nz4();
    }

    @tp4(version = "1.3")
    public static /* synthetic */ void l() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void q() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void r() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void s() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void v() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void w() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void x() {
    }

    @tp4(version = "1.1")
    public static /* synthetic */ void z() {
    }

    @Override // com.p7700g.p99005.u55
    @NotNull
    public Collection<u55<?>> A() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean B4() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean D1() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @NotNull
    public Collection<z55<Object>> E() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @tp4(version = "1.1")
    public boolean G3(@Nullable Object obj) {
        return s.c(obj, c());
    }

    @Override // com.p7700g.p99005.u55
    public boolean H() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @NotNull
    public List<u55<? extends Object>> N() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean R() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean S() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @Nullable
    public String T() {
        return s.b(c());
    }

    @Override // com.p7700g.p99005.u55
    @NotNull
    public List<j65> V() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.l15
    @NotNull
    public Class<?> c() {
        return this.y;
    }

    @Override // com.p7700g.p99005.u55
    public boolean equals(@Nullable Object obj) {
        return (obj instanceof m15) && c25.g(yy4.g(this), yy4.g((u55) obj));
    }

    @Override // com.p7700g.p99005.s55
    @NotNull
    public List<Annotation> getAnnotations() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @Nullable
    public n65 getVisibility() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean h() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public int hashCode() {
        return yy4.g(this).hashCode();
    }

    @Override // com.p7700g.p99005.u55
    public boolean isOpen() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean j3() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @NotNull
    public List<k65> m() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    public boolean o() {
        k();
        throw new lo4();
    }

    @NotNull
    public String toString() {
        return c().toString() + c35.b;
    }

    @Override // com.p7700g.p99005.u55, com.p7700g.p99005.y55
    @NotNull
    public Collection<t55<?>> u() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @Nullable
    public Object w0() {
        k();
        throw new lo4();
    }

    @Override // com.p7700g.p99005.u55
    @Nullable
    public String y() {
        return s.a(c());
    }
}