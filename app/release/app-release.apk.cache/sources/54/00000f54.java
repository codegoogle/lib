...
public final class C04938r {
    ...
    public static void A03() {
        A06 = new byte[]{-97, -12, -16, -14, -14, -12, -5, -12, 1, -2, -4, -12, 3, -12, 1, -18, -67, -65, -48, -59, -46, -59, -48, -43, 41, 54, 44, 58, 55, 49, 44, -10, 49, 54, 60, 45, 54, 60, -10, 41, 43, 60, 49, 55, 54, -10, 10, 9, 28, 28, 13, 26, 33, 39, Flags.CD, 16, 9, 22, 15, 13, 12, 23, 44, 23, 31, 34, 23, 24, 34, 27, 21, 35, 27, 35, 37, 40, 47, 33, 32, 51, 51, 36, 49, 56, -4, 1, -6, Flags.CD, 0, 2, 7, 0, 35, 47, 34, 34, 28, 48, 45, 30, 32, 34, -18, -20, -11, -20, -7, -16, -22, 19, 12, 29, 12, 19, -60, -63, -58, -77, -58, -69, -63, -64, -79, -60, -76, -78, -67, -74, 4, -10, -1, 4, 0, 3, 8, 9, -10, 9, 10, 8, 45, 40, 45, 26, 37, 24, 38, 30, 38, 40, 43, 50, 16, 17, -49};
    }
    ...
    /* JADX WARN: Type inference failed for: r6v0, types: [com.facebook.ads.redexgen.X.8H] */
    /* JADX WARN: Type inference failed for: r6v1, types: [com.facebook.ads.redexgen.X.8H] */
    /* JADX WARN: Type inference failed for: r6v2, types: [android.content.Intent] */
    public static void A0A(C8H c8h) {
        try {
            c8h = c8h.registerReceiver(null, new IntentFilter(A01(24, 37, 122)));
            if (c8h == 0) {
                return;
            }
            int status = c8h.getIntExtra(A01(109, 5, 89), -1);
            int level = c8h.getIntExtra(A01(123, 5, 3), -1);
            int intExtra = c8h.getIntExtra(A01(WKSRecord.Service.INGRES_NET, 6, 71), -1);
            boolean z = intExtra == 2 || intExtra == 5;
            float f = 0.0f;
            if (level > 0) {
                float batteryLevel = level;
                f = (status / batteryLevel) * 100.0f;
            }
            A05.put(A01(77, 7, 113), String.valueOf(f));
            A05.put(A01(84, 8, 75), z ? A01(1, 1, 117) : A01(0, 1, 33));
        } catch (IllegalArgumentException e) {
            c8h.A04().A82(A01(102, 7, 57), C04848i.A1B, new C04858j(e));
        }
    }

    public static synchronized void A0B(C8H c8h) {
        synchronized (C04938r.class) {
            A09(c8h);
            A0A(c8h);
            if (A04 == null) {
                A04 = (SensorManager) c8h.getSystemService(A01(128, 6, 67));
                if (A04 == null) {
                    return;
                }
            }
