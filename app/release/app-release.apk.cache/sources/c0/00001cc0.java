...
public final class zzaoc {
    ...
    private final Uri zzg(Uri uri, String str) throws zzaod {
        ...
        try {
            try {
                if (uri.getHost().equals(this.zzb)) {
                    if (!uri.toString().contains("dc_ms=")) {
                        ...
                        if (indexOf != -1) {
                            int i = indexOf + 1;
                            return Uri.parse(uri2.substring(0, i) + "dc_ms" + AppCenter.KEY_VALUE_DELIMITER + str + AppCenter.PAIR_DELIMITER + uri2.substring(i));
                        }
                        String encodedPath = uri.getEncodedPath();
                        int indexOf2 = uri2.indexOf(encodedPath);
                        return Uri.parse(uri2.substring(0, encodedPath.length() + indexOf2) + AppCenter.PAIR_DELIMITER + "dc_ms" + AppCenter.KEY_VALUE_DELIMITER + str + AppCenter.PAIR_DELIMITER + uri2.substring(indexOf2 + encodedPath.length()));
                    }
            ...
            if (uri.getQueryParameter("ms") == null) {
                ...
                if (indexOf3 != -1) {
                    int i2 = indexOf3 + 1;
                    return Uri.parse(uri3.substring(0, i2) + "ms" + AppCenter.KEY_VALUE_DELIMITER + str + "&" + uri3.substring(i2));
                }
    ...
    public final Uri zza(Uri uri, Context context, View view, Activity activity) throws zzaod {
        try {
            return zzg(uri, this.zzd.zzf(context, uri.getQueryParameter(b.cZ), view, activity));
