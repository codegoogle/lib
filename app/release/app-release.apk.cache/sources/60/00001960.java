...
public final class zaaa implements zaca {
    ...
    private zaaa(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<Api.AnyClientKey<?>, Api.Client> map, Map<Api.AnyClientKey<?>, Api.Client> map2, ClientSettings clientSettings, Api.AbstractClientBuilder<? extends com.google.android.gms.signin.zae, SignInOptions> abstractClientBuilder, @z1 Api.Client client, ArrayList<zat> arrayList, ArrayList<zat> arrayList2, Map<Api<?>, Boolean> map3, Map<Api<?>, Boolean> map4) {
        this.zaa = context;
        this.zab = zabeVar;
        this.zam = lock;
        this.zac = looper;
        ...
        for (Api.AnyClientKey<?> anyClientKey : map2.keySet()) {
            q7Var.put(anyClientKey, this.zad);
        }
        for (Api.AnyClientKey<?> anyClientKey2 : map.keySet()) {
            q7Var.put(anyClientKey2, this.zae);
        }
        this.zaf = Collections.unmodifiableMap(q7Var);
    }
    ...
    private static boolean zaE(@z1 ConnectionResult connectionResult) {
    ...
    public static zaaa zag(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<Api.AnyClientKey<?>, Api.Client> map, ClientSettings clientSettings, Map<Api<?>, Boolean> map2, Api.AbstractClientBuilder<? extends com.google.android.gms.signin.zae, SignInOptions> abstractClientBuilder, ArrayList<zat> arrayList) {
        ...
        for (Map.Entry<Api.AnyClientKey<?>, Api.Client> entry : map.entrySet()) {
            ...
            if (value.requiresSignIn()) {
                q7Var.put(entry.getKey(), value);
            } else {
                q7Var2.put(entry.getKey(), value);
            }
        }
        Preconditions.checkState(!q7Var.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        q7 q7Var3 = new q7();
        q7 q7Var4 = new q7();
        for (Api<?> api : map2.keySet()) {
            Api.AnyClientKey<?> zab = api.zab();
            if (q7Var.containsKey(zab)) {
                q7Var3.put(api, map2.get(api));
            } else if (q7Var2.containsKey(zab)) {
                q7Var4.put(api, map2.get(api));
            } else {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (q7Var3.containsKey(zatVar.zaa)) {
                arrayList2.add(zatVar);
            } else if (q7Var4.containsKey(zatVar.zaa)) {
                arrayList3.add(zatVar);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        return new zaaa(context, zabeVar, lock, looper, googleApiAvailabilityLight, q7Var, q7Var2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, q7Var3, q7Var4);
    }
    ...
    @Override // com.google.android.gms.common.api.internal.zaca
    @GuardedBy("mLock")
    public final ConnectionResult zac(long j, @x1 TimeUnit timeUnit) {
        throw new UnsupportedOperationException();
    }
    ...
    public final void zas(String str, @z1 FileDescriptor fileDescriptor, PrintWriter printWriter, @z1 String[] strArr) {
        ...
        this.zae.zas(String.valueOf(str).concat(g54.A), fileDescriptor, printWriter, strArr);
        printWriter.append((CharSequence) str).append("anonClient").println(":");
        this.zad.zas(String.valueOf(str).concat(g54.A), fileDescriptor, printWriter, strArr);
    }
